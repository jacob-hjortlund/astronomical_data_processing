% Define document class
\documentclass{aastex631}
\usepackage{showyourwork}

% Begin!
\begin{document}

% Title
\title{An open source scientific article}

% Author list
\author{@jacob-hjortlund}

% Abstract with filler text
\begin{abstract}
    Lorem ipsum dolor sit amet, consectetuer adipiscing elit.
    Ut purus elit, vestibulum ut, placerat ac, adipiscing vitae, felis.
    Curabitur dictum gravida mauris, consectetuer id, vulputate a, magna.
    Donec vehicula augue eu neque, morbi tristique senectus et netus et.
    Mauris ut leo, cras viverra metus rhoncus sem, nulla et lectus vestibulum.
    Phasellus eu tellus sit amet tortor gravida placerat.
    Integer sapien est, iaculis in, pretium quis, viverra ac, nunc.
    Praesent eget sem vel leo ultrices bibendum.
    Aenean faucibus, morbi dolor nulla, malesuada eu, pulvinar at, mollis ac.
    Curabitur auctor semper nulla donec varius orci eget risus.
    Duis nibh mi, congue eu, accumsan eleifend, sagittis quis, diam.
    Duis eget orci sit amet orci dignissim rutrum.
\end{abstract}

% Main body with filler text
\section{Introduction}
\label{sec:intro}

Lorem ipsum dolor sit amet, consectetuer adipiscing elit.
Ut purus elit, vestibulum ut, placerat ac, adipiscing vitae, felis.
Curabitur dictum gravida mauris, consectetuer id, vulputate a, magna.
Donec vehicula augue eu neque, morbi tristique senectus et netus et.
Mauris ut leo, cras viverra metus rhoncus sem, nulla et lectus vestibulum.
Phasellus eu tellus sit amet tortor gravida placerat.
Integer sapien est, iaculis in, pretium quis, viverra ac, nunc.
Praesent eget sem vel leo ultrices bibendum.
Aenean faucibus, morbi dolor nulla, malesuada eu, pulvinar at, mollis ac.
Curabitur auctor semper nulla donec varius orci eget risus.
Duis nibh mi, congue eu, accumsan eleifend, sagittis quis, diam.
Duis eget orci sit amet orci dignissim rutrum.

Nam dui ligula, fringilla a, euismod sodales, sollici- tudin vel, wisi.
Morbi auctor lorem non justo, nam lacus libero, pretium at, lobortis vitae.
Donec aliquet, tortor sed accumsan bibendum, erat ligula aliquet magna.
Morbi ac orci et nisl hendrerit mollis, suspendisse ut massa, cras nec ante.
Pellentesque a nulla cum sociis natoque penatibus et magnis dis parturient.
Aliquam tincidunt urna, nulla ullamcorper vestibulum turpis.
Pellentesque cursus luctus mauris \citep{Luger2021}.

\begin{figure}[ht!]
    \script{random_bias_frames.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/random_bias_frames.pdf}
        \caption{
            Plot showing 4 random bias frames.
        }
        \label{fig:random_bias}
    \end{centering}
\end{figure}

\begin{figure}[ht!]
    \script{first_ron_estimates.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/bias_frame_distributions.pdf}
        \caption{
            INSTERT CAPTION
        }
        \label{fig:bias_frame_distributions}
    \end{centering}
\end{figure}

\begin{figure}[ht!]
    \script{master_biases_stats_figure.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/bias_frame_stats.pdf}
        \caption{
            Master Bias frames created using 1-40 frames. Outliers excluded using $\sigma$-clipping with median and MAD-STD with $\pm3\sigma$-thresholds. Frames combined using averaging.
        }
        \label{fig:bias_frame_stats}
    \end{centering}
\end{figure}

\begin{figure}[ht!]
    \script{master_bias_figure.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/master_bias.pdf}
        \caption{
            Master Bias frame created using 40 frames. Outliers excluded using $\sigma$-clipping with median and MAD-STD with $\pm3\sigma$-thresholds. Frames combined using averaging.
        }
        \label{fig:master_bias}
    \end{centering}
\end{figure}

\begin{figure}[ht!]
    \script{master_dark_figure.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/master_dark.pdf}
        \caption{
            Plot showing 4 random bias frames.
        }
        \label{fig:master_dark}
    \end{centering}
\end{figure}

\begin{figure}[ht!]
    \script{master_skyflats_figure.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/master_skyflat.pdf}
        \caption{
            Plot showing 4 random bias frames.
        }
        \label{fig:master_dark}
    \end{centering}
\end{figure}

\begin{figure}[ht!]
    \script{master_lampflats_figure.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/master_lampflat.pdf}
        \caption{
            Plot showing 4 random bias frames.
        }
        \label{fig:master_dark}
    \end{centering}
\end{figure}

\begin{figure}[ht!]
    \script{science_sky_image_figure.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/science_sky_cal.pdf}
        \caption{
            Plot showing 4 random bias frames.
        }
        \label{fig:master_dark}
    \end{centering}
\end{figure}

\begin{figure}[ht!]
    \script{science_lamp_image_figure.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/science_lamp_cal.pdf}
        \caption{
            Plot showing 4 random bias frames.
        }
        \label{fig:master_dark}
    \end{centering}
\end{figure}

\bibliography{bib}

\end{document}
